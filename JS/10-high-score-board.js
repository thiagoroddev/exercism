...
console.log(addPlayer({ 'Dave Thomas': 2 }, 'José Valim', 4))
...
function updateScore(scoreBoard, player, points) {
  if (scoreBoard.hasOwnProperty(player)) {
      let placarAtualizado = scoreBoard[player] += points
  } else {
...
function normalizeScore(params) {
  let pontuacao = params['score']
  let funcaoNormaliza = params['normalizeFunction']

  let pontuacaoNomalizada = funcaoNormaliza(pontuacao)

  return pontuacaoNomalizada
...
function normalizeScore(params) {
  ...
  const correctedScore = normalizeFunction(score);

...
{
    // Tarefa 1
    public static Dictionary<string, int> CreateScoreBoard()
    {
        return new Dictionary<string, int> { { "The Best Ever", 1000000 } };
    }

    // Tarefa 2
    public static Dictionary<string, int> AddPlayer(Dictionary<string, int> scoreBoard, string player, int score)
    {
    ...
    // Tarefa 3
    public static Dictionary<string, int> RemovePlayer(Dictionary<string, int> scoreBoard, string player)
    {
    ...
    // Tarefa 4
    public static Dictionary<string, int> UpdateScore(Dictionary<string, int> scoreBoard, string player, int points)
    {
        if (scoreBoard.ContainsKey(player))
        {
    ...
    // Tarefa 5
    public static Dictionary<string, int> ApplyMondayBonus(Dictionary<string, int> scoreBoard)
    {
        foreach (var player in scoreBoard.Keys.ToList())
        {
    ...
    // Tarefa 6
    public static int NormalizeScore(int score, Func<int, int> normalizeFunction)
    {
        return normalizeFunction(score);
    }

    // Exemplo de função de normalização
    ...
    public static void Main(string[] args)
    {
        ...
        Console.WriteLine("Initial Scoreboard: ");
        PrintScoreboard(scoreBoard);

        scoreBoard = AddPlayer(scoreBoard, "José Valim", 486373);
        Console.WriteLine("Scoreboard after adding player: ");
        PrintScoreboard(scoreBoard);

        scoreBoard = RemovePlayer(scoreBoard, "Dave Thomas");
        Console.WriteLine("Scoreboard after removing player: ");
        PrintScoreboard(scoreBoard);

        scoreBoard = UpdateScore(scoreBoard, "Freyja Ćirić", 73);
        Console.WriteLine("Scoreboard after updating score: ");
        PrintScoreboard(scoreBoard);

        scoreBoard = ApplyMondayBonus(scoreBoard);
        Console.WriteLine("Scoreboard after applying Monday bonus: ");
        PrintScoreboard(scoreBoard);

        int normalizedScore = NormalizeScore(400, Normalize);
        Console.WriteLine("Normalized Score: " + normalizedScore);
    }

    public static void PrintScoreboard(Dictionary<string, int> scoreBoard)
    {

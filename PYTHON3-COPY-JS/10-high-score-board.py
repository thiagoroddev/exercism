...
# Tarefa 2
def add_player(score_board, player, score):
    score_board[player] = score
    return score_board

# Example usage:
print(add_player({'Dave Thomas': 2}, 'José Valim', 4))

# Tarefa 3
def remove_player(score_board, player):
    del score_board[player]
    return score_board

# Example usage:
# print(remove_player({'Dave Thomas': 0}, 'Dave Thomas'))
# print(remove_player({'Dave Thomas': 0}, 'Rose Fanaras'))

# Tarefa 4
def update_score(score_board, player, points):
    if player in score_board:
        score_board[player] += points
    else:
        score_board[player] = points
    return score_board

# Example usage:
# print(update_score({'Freyja Ćirić': 12771008}, 'Freyja Ćirić', 73))
...
def apply_monday_bonus(score_board):
    for player in score_board:
        score_board[player] += 100
    return score_board

# Example usage:
# print(apply_monday_bonus(score_board))
...
# Como eu fiz
def normalize_score(params):
    score = params['score']
    normalize_function = params['normalizeFunction']
    normalized_score = normalize_function(score)
    return normalized_score

# Como o Copilot fez
def normalize_score(params):
    score, normalize_function = params['score'], params['normalizeFunction']
    corrected_score = normalize_function(score)
    return corrected_score

# Example usage:
# print(normalize_score(params))

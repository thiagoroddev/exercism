...
def time_to_mix_juice(name):
    ...
    return switch_dict.get(name, 2.5)

# Example usage:
juice_name = 'Pure Strawberry Joy'
result = time_to_mix_juice(juice_name)
print(result)


def limes_to_cut(wedges_needed, limes):
    accumulator = 0
    wedges_limes = 0

    while wedges_limes < wedges_needed and accumulator < len(limes):
        lime_size = limes[accumulator]

        if lime_size == 'small':
            wedges_limes += 6
            print('+6 fatias')
        elif lime_size == 'medium':
            wedges_limes += 8
            print('+8 fatias')
        elif lime_size == 'large':
            wedges_limes += 10
            print('+10 fatias')
...
# Example usage:
wedges_needed = 25
lime_sizes = ['small', 'small', 'large', 'medium', 'small']
result = limes_to_cut(wedges_needed, lime_sizes)
print(result)



# Como eu fiz
def remaining_orders(time_left, orders):
    time = 0
    juices_missings = list(orders)
    ...
    for juice in orders:
        ...
        if time < time_left:
            if juice == 'Pure Strawberry Joy':
                time += 0.5
                juices_missings.pop(0)
                print('add 0.5')
            elif juice == 'Energizer' or juice == 'Green Garden':
                time += 1.5
                juices_missings.pop(0)
                print('add 1.5')
            elif juice == 'Tropical Island':
                time += 3
                juices_missings.pop(0)
                print('add 3')
            elif juice == 'All or Nothing':
                time += 5
                juices_missings.pop(0)
                print('add 5')
            else:
    ...
    return juices_missings

# Example usage:
time_left = 5
juice_orders = ['Energizer', 'All or Nothing', 'Green Garden']
result = remaining_orders(time_left, juice_orders)
print(result)
